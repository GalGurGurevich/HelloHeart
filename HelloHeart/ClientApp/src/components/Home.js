...
export default function Home() {
  ...
  const textInputError = testInput && testInput.length > 1 && !regValidate(testInput);

  const header = `Hello Heart`
  const errorTxt = `Please avoid typing symbols`
  ...
  return (
    <div className="home-container">
      ...
      <form className="input-test-form" onSubmit={onSubmit}>
        ...
        <input className="input-field" type='text'value={testInput} onChange={e => setTestInput(e.target.value)} placeholder="what should we check today?" required={true}></input>
        <label className="label-header">Testing Numiric Result: </label>
        <input className="input-field" type='number' value={testNumber} onChange={e => setTestNumber(e.target.value)} placeholder={0} required={true}></input>
        <button type='submit'>Submit Result</button>
      </form>

      {textInputError && <span className="error-line">{errorTxt}</span>}
